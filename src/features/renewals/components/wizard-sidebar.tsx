...
export function WizardSidebar({
  ...
  return (
    <div className="flex flex-col gap-5 z-10 w-full">
      <div>
        <h2 className="text-xl font-black tracking-tight leading-none text-foreground">Renovacion</h2>
        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">Membresia Gym</p>
      </div>

      <div className="flex flex-col items-center justify-center select-none py-2">
        <p className="text-[9px] font-black tracking-widest text-primary/95 uppercase mb-3.5">Renovación</p>
        <svg className="h-16 w-auto text-primary" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
          ...
          <circle cx="50" cy="50" r="40" stroke="currentColor" strokeWidth="1" strokeDasharray="2 4" className="opacity-15" />
          <circle cx="50" cy="50" r="30" stroke="currentColor" strokeWidth="1" strokeDasharray="3 3" className="opacity-25" />
          <g className="animate-spin" style={{ transformOrigin: '50px 50px', animationDuration: '2s' }}>
            <path d="M 50,20 A 30,30 0 0,1 80,50 L 74,50" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
            <path d="M 80,50 L 84,42" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
            <path d="M 80,50 L 72,44" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
            <path d="M 50,80 A 30,30 0 0,1 20,50 L 26,50" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
            <path d="M 20,50 L 16,58" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
            <path d="M 20,50 L 28,56" stroke="url(#renewGrad)" strokeWidth="3" strokeLinecap="round" />
          </g>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground">Búsqueda CARNET</span>
        </div>
      ...
      {searchQuery.length >= 2 && (
        <div className="space-y-2.5">
          <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground px-1">Resultado de Busqueda:</p>
          <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1 scrollbar-none">
            {searchingMembers ? (
              ...
              <p className="text-xs text-muted-foreground text-center py-4">No se encontraron socios</p>
            ) : (
              memberSearchResults.map((m) => {
                ...
                return (
                  ...
                  >
                    {m.photoUrl ? (
                      <img src={m.photoUrl} alt={m.fullName} className="size-8 rounded-full object-cover shrink-0 border border-foreground/10 shadow-sm" />
                    ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-bold text-xs truncate leading-tight">{m.fullName}</p>
                      <p className="text-[9px] font-semibold text-muted-foreground mt-0.5">CI: {m.documentNumber || '—'}</p>
                    </div>
                    {isSelected && (
          ...
          return (
            <div key={s.id} className="relative flex gap-3.5 group">
              ...
              <div className="min-w-0 pt-0.5">
                <p className={`text-xs font-bold transition-colors ${isActive ? 'text-foreground font-black' : 'text-muted-foreground/80'}`}>
                  {s.label}
                </p>
                <p className="text-[10px] text-muted-foreground/60 font-semibold mt-0.5">{s.sublabel}</p>
              </div>
            </div>
          )
        })}
