...
export function MemberHeaderCard({ member, extraInfo }: MemberHeaderCardProps) {
  return (
    <div className="bg-muted/50 rounded-2xl p-4.5 border border-border/10 flex flex-col items-center justify-center text-center gap-3 w-full">
      {member.photoUrl ? (
        <img src={member.photoUrl} alt={member.fullName} className="size-12 rounded-full object-cover shrink-0 border border-foreground/10 shadow-sm" />
      ) : (
      ...
      <div className="min-w-0">
        <p className="font-bold text-sm leading-tight text-foreground">{member.fullName}</p>
