...
import { eq, desc, and, lte, gte, sql, count as drizzleCount } from 'drizzle-orm'
...
export const getNotifications = createServerFn({ method: 'GET' })
  .inputValidator((data: { page?: number; pageSize?: number }) =>
    z.object({ page: z.number().optional().default(1), pageSize: z.number().optional().default(50) }).parse(data),
  )
  .handler(async ({ data }) => {
...
export const generateNotifications = createServerFn({ method: 'POST' })
  ...
  const expiringSubs = await db.query.subscriptions.findMany({
    where: and(eq(subscriptions.status, 'ACTIVE'), lte(subscriptions.endDate, futureDate), gte(subscriptions.endDate, now)),
    with: { member: true },
  })

  for (const sub of expiringSubs) {
    const existing = await db.query.notifications.findFirst({
      where: and(eq(notifications.type, 'EXPIRATION'), eq(notifications.referenceId, sub.id)),
    })
    if (!existing) {
      newNotifications.push({
        type: 'EXPIRATION',
        title: 'Suscripción próxima a vencer',
        message: `La suscripción de ${sub.member?.fullName ?? 'miembro'} vence el ${sub.endDate.toLocaleDateString('es-AR')}`,
        referenceId: sub.id,
  ...
  const lowStockProducts = await db
    .select()
    .from(products)
    .where(and(eq(products.isActive, true), sql`${products.stockCurrent} <= ${products.stockMinimum}`))

  for (const product of lowStockProducts) {
    const existing = await db.query.notifications.findFirst({
      where: and(eq(notifications.type, 'LOW_STOCK'), eq(notifications.referenceId, product.id)),
    })
    if (!existing) {
      newNotifications.push({
        type: 'LOW_STOCK',
