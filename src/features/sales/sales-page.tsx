...
export function SalesPage() {
  ...
  const [selectedSale, setSelectedSale] = useState<typeof salesList[number] | null>(null)
  ...
  const filteredSales = salesList.filter((sale: typeof salesList[number]) => {
    ...
    const client = (sale.member?.fullName || sale.customerName || '').toLowerCase()
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        columns={[
          { key: 'saleNumber', label: 'N° Venta', render: (sale: typeof salesList[number]) => <span className="font-mono text-sm font-semibold">{sale.saleNumber}</span> },
          { key: 'date', label: 'Fecha', render: (sale: typeof salesList[number]) => <span className="text-xs">{formatDateTime(sale.soldAt)}</span> },
          { key: 'client', label: 'Cliente / Socio', render: (sale: typeof salesList[number]) => (
            sale.member
              ? <div className="flex flex-col"><span className="font-medium text-primary">{sale.member.fullName}</span><span className="text-[10px] text-muted-foreground">Socio #{sale.member.id}</span></div>
              : <span>{sale.customerName || 'Cliente General'}</span>
          )},
          { key: 'seller', label: 'Atendido por', render: (sale: typeof salesList[number]) => <span className="text-xs text-muted-foreground">{sale.user?.name}</span> },
          { key: 'method', label: 'Método', render: (sale: typeof salesList[number]) => <Badge variant="outline">{sale.paymentMethod}</Badge> },
          { key: 'total', label: 'Total', render: (sale: typeof salesList[number]) => <span className="font-bold text-primary">${sale.total}</span> },
          { key: 'actions', label: 'Detalles', className: 'text-right', render: (sale: typeof salesList[number]) => (
          ...
          {selectedSale && (
            <div className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-2 text-xs border-b pb-3 font-mono">
                ...
                <div>
                  <span className="text-muted-foreground block font-sans">Cliente:</span>
                  <span>{selectedSale.member?.fullName || selectedSale.customerName || 'Cliente General'}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block font-sans">Vendedor:</span>
                  <span>{selectedSale.user?.name}</span>
                </div>
                <div>
                  <span className="text-muted-foreground block font-sans">Método de Pago:</span>
              ...
              <div>
                ...
                <div className="border rounded-lg overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {selectedSale.items?.map((item: NonNullable<typeof salesList[number]['items']>[number]) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="py-2 text-right text-xs">${item.unitPrice}</TableCell>
