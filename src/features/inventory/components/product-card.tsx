...
export function ProductCard({ product, trend, isAdmin, trendDays, onEdit, onAdjust }: ProductCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-muted-foreground font-semibold mb-2">{product.category.name}</p>

        <div className="flex items-baseline gap-3">
          <div>
            <p className="text-[9px] text-muted-foreground uppercase font-bold">Venta</p>
            <p className="text-base font-black text-primary">{formatCurrency(Number(product.salePrice))}</p>
          </div>
          <div>
            <p className="text-[9px] text-muted-foreground uppercase font-bold">Compra</p>
            <p className="text-sm font-bold text-muted-foreground">{formatCurrency(Number(product.purchasePrice))}</p>
          </div>
        </div>
      </div>

      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border/5">
        <Button size="sm" variant="outline" className="flex-1 h-8 rounded-xl text-xs font-bold" onClick={() => onAdjust(product)}>
          <ArrowUpDown className="size-3" /> Stock
        </Button>
        {isAdmin && (
          <Button size="sm" variant="ghost" className="h-8 rounded-xl" onClick={() => onEdit(product)}>
