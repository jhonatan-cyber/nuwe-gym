...
const STEPS = [
  { number: 1, label: 'Información Personal', subtitle: 'Registre los datos de la persona' },
  { number: 2, label: 'Selección de Plan', subtitle: 'Elija un plan de inscripción' },
  { number: 3, label: 'Pago de Inscripción', subtitle: 'Seleccione un método de pago' },
  { number: 4, label: 'Enrolamiento Facial', subtitle: 'Captura Datos Biometricos' },
...
export function WizardSidebar({ step, variant = 'dialog', onClose, insideLayout = false }: WizardSidebarProps) {
  const content = (
    <>
      ...
      {variant !== 'inline' && (
        <Button size="icon" variant="ghost" onClick={onClose} className="absolute top-4 right-4 z-10">
          <X className="size-4" />
        </Button>
      )}
      <div className="mb-6 z-10 w-full">
        <ToggleGroup
          type="single"
          value="enroll"
          onValueChange={(v) => { if (v === 'list') onClose() }}
        >
      ...
      <nav className="flex-1 space-y-5 z-10">
        {STEPS.map((s, idx) => {
          ...
          return (
            <div key={s.number} className="flex items-start gap-4 relative pb-2">
              {idx < STEPS.length - 1 && (
                <div
                  className={`absolute left-[18px] top-10 bottom-0 w-0.5 -translate-x-1/2 rounded-full transition-colors duration-300 ${
                    isCompleted ? 'bg-foreground' : 'dark:bg-white/10 bg-black/10'
                  }`}
                />
              )}
              <div
              ...
              >
                {isCompleted ? <Check className="size-4 stroke-[3]" /> : <span className="text-sm font-bold">{s.number}</span>}
              </div>
              <div className="pt-1 flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p
                    className={`text-sm font-bold leading-tight transition-colors duration-300 ${
                      isActive ? 'text-foreground font-black' : isCompleted ? 'text-muted-foreground hover:text-foreground' : 'text-muted-foreground/60'
                    }`}
                  >
                    {s.label}
                  </p>
                  {isActive && <span className="size-2 rounded-full bg-foreground animate-pulse shrink-0 shadow-sm" />}
                </div>
                <p className="text-[10px] font-medium text-muted-foreground/50 mt-0.5 truncate">{s.subtitle}</p>
              </div>
            </div>
          )
        })}
