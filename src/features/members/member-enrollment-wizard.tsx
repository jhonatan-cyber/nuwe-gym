...
const STEPS_LABELS = ['Información Personal', 'Selección de Plan', 'Pago de Inscripción', 'Enrolamiento Facial']
...
export function MemberEnrollmentWizard({ isOpen, onClose, variant = 'dialog', onRenewalClick }: WizardProps) {
  const queryClient = useQueryClient()
  const { stream: cameraStream, videoRef, start: startCamera, capture: capturePhoto, setStatus: setCameraStatus } = useCameraCapture()

  const [step, setStep] = useState<Step>(1)
  const [personalInfo, setPersonalInfo] = useState<PersonalInfoState>({
    firstName: '', lastName: '', documentNumber: '', phone: '', gender: 'MALE', birthDate: '', photoBase64: null,
  })
  ...
  const [enrollmentMode, setEnrollmentMode] = useState<'CAMERA' | 'BIOMETRIC'>('CAMERA')
  ...
  const validateStep = useCallback((currentStep: Step): boolean => {
    ...
    if (currentStep === 1) {
      if (!personalInfo.firstName.trim()) newErrors.firstName = 'El nombre es obligatorio'
      if (!personalInfo.lastName.trim()) newErrors.lastName = 'El apellido es obligatorio'
      if (!personalInfo.documentNumber.trim()) newErrors.documentNumber = 'El documento es obligatorio'
    }
  ...
  const simulateBiometricCapture = async () => {
    ...
    await new Promise((r) => setTimeout(r, 1200))
    setBiometricStatus('Esperando que el socio se posicione frente al sensor...')
    ...
    setPersonalInfo((prev) => ({ ...prev, photoBase64: `data:image/svg+xml;utf8,${encodeURIComponent(mockSvg)}` }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (personalInfo.photoBase64) {
        await uploadMemberPhoto({ data: { memberId: member.id, photoBase64: personalInfo.photoBase64 } })
      }
      ...
      toast.error(err instanceof Error ? err.message : 'Error al registrar el socio')
    } finally {
  ...
  const handleSavePhoto = async () => {
    if (!createdMemberId || !personalInfo.photoBase64) { setStep(5); return }
    setIsSavingPhoto(true)
    try {
      await uploadMemberPhoto({ data: { memberId: createdMemberId, photoBase64: personalInfo.photoBase64 } })
      toast.success('Foto de perfil vinculada correctamente')
      queryClient.invalidateQueries({ queryKey: ['members'] })
      setStep(5)
    } catch { toast.error('Error al guardar la foto') }
  ...
  const goBack = () => { if (step > 1) setStep((s) => Math.max(s - 1, 1) as Step) }

  const resetWizard = () => {
    setStep(1)
    setPersonalInfo({ firstName: '', lastName: '', documentNumber: '', phone: '', gender: 'MALE', birthDate: '', photoBase64: null })
  ...
  const sendWhatsAppMessage = () => {
    ...
    const priceFormatted = selectedPackage ? formatCurrency(Number(selectedPackage.price)) : ''
  ...
  useEffect(() => {
    if (step === 4 && enrollmentMode === 'CAMERA' && !personalInfo.photoBase64 && cameraStream) {
      setCameraStatus('Buscando rostro...')
      const t1 = setTimeout(() => {
        setCameraStatus('Rostro enfocado. Capturando en 2s...')
        setTimeout(() => { capturePhoto(); setCameraStatus('') }, 2000)
      }, 1200)
      return () => clearTimeout(t1)
    }
  }, [step, enrollmentMode, cameraStream, personalInfo.photoBase64, capturePhoto, setCameraStatus])

  useEffect(() => {
    if (step === 4 && enrollmentMode === 'BIOMETRIC' && !personalInfo.photoBase64 && !isConnectingDevice) {
  ...
  const stepContent = () => {
    switch (step) {
      case 1: return <Step1PersonalInfo state={personalInfo} errors={errors} onChange={setPersonalInfo} onRenewalClick={onRenewalClick} />
      case 2: return <Step2PackageSelect packages={packages} selectedPackageId={selectedPackageId} error={errors.selectedPackageId} onSelect={setSelectedPackageId} />
      case 3: return <Step3Payment selectedPackage={selectedPackage} paymentMethod={paymentMethod} onMethodChange={setPaymentMethod} />
      case 4: return (
        <Step4FacialEnrollment
          ...
          readerIp={readerIp}
          videoRef={videoRef}
          onModeChange={(mode) => { setPersonalInfo((p) => ({ ...p, photoBase64: null })); setEnrollmentMode(mode) }}
          onReaderIpChange={setReaderIp}
          onStartCamera={startCamera}
          onCapturePhoto={() => { const p = capturePhoto(); if (p) setPersonalInfo((prev) => ({ ...prev, photoBase64: p })) }}
          onSimulateBiometric={simulateBiometricCapture}
          onRecapture={() => { setPersonalInfo((p) => ({ ...p, photoBase64: null })); if (enrollmentMode === 'CAMERA') startCamera() }}
        />
      )
      case 5: return <Step5Complete createdMemberName={createdMemberName} createdPlanName={createdPlanName} paymentMethod={paymentMethod} onSendWhatsApp={sendWhatsAppMessage} onFinish={handleClose} />
    }
  }
            ...
            {step < 5 && (
              <div className="flex items-center gap-1.5 mb-5">
                {STEPS_LABELS.map((_, i) => (
                  <div key={i} className={`h-1 rounded-full transition-all flex-1 ${i + 1 <= step ? 'bg-gray-900 dark:bg-white' : 'bg-gray-200 dark:bg-white/10'}`} />
                ))}
              </div>
            )}
            <div className="flex-1 min-h-0 overflow-y-auto text-gray-900 dark:text-gray-100">{stepContent()}</div>
            <WizardFooter step={step} isSubmitting={isSubmitting} isSavingPhoto={isSavingPhoto} hasPhoto={!!personalInfo.photoBase64} onNext={goNext} onBack={goBack} onSavePhoto={handleSavePhoto} onSkipPhoto={() => setStep(5)} />
          </div>
        </div>
      </div>
    )
        ...
        <ModuleLayout
          breadcrumb={
            <div className="flex items-center gap-1">
              <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer" onClick={handleClose}>Inscripciones</span>
              <ChevronRight className="size-3 text-muted-foreground/50" />
              <span className="text-foreground">Inscripcion</span>
            </div>
          }
          title="Inscripciones"
          leftPanel={<WizardSidebar step={step} variant={variant} onClose={handleClose} insideLayout />}
        >
          {formContent}
        </ModuleLayout>
      )
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-4xl p-0 gap-0 overflow-hidden rounded-2xl" showCloseButton={false} onInteractOutside={(e) => e.preventDefault()}>
        {renderWizardContent()}
      </DialogContent>
    </Dialog>
  )
