        ...
        <LazyRecharts height={220}>
          {(R) => (
            <R.ResponsiveContainer width="100%" height="100%">
              <R.LineChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                <R.CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <R.XAxis dataKey="date" className="text-xs text-muted-foreground" tick={{ fontSize: 10 }} />
                <R.YAxis domain={[minW, maxW]} className="text-xs text-muted-foreground" tick={{ fontSize: 10 }} unit=" kg" />
                <R.Tooltip
                  contentStyle={{ borderRadius: '10px', border: '1px solid hsl(var(--border))', fontSize: 12 }}
                  formatter={(value) => [`${value} kg`, 'Peso']}
                />
                <R.ReferenceLine y={avg} stroke="#6366f1" strokeDasharray="4 4"
