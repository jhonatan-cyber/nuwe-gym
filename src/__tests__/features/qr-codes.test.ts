...
describe('QR Codes', () => {
  ...
  it('should find members with QR code', async () => {
    ...
    await db
      .update(members)
      .set({ qrCode: 'qr-abc' })
      .where(eq(members.id, m1.id))
    await createMember()

    const withQr = await db
      .select()
      .from(members)
      .where(isNotNull(members.qrCode))
  ...
  it('should find members without QR code', async () => {
    ...
    const withoutQr = await db
      .select()
      .from(members)
      .where(isNull(members.qrCode))
  ...
  it('should process a QR check-in', async () => {
    ...
    await db
      .update(members)
      .set({ qrCode: 'qr-checkin' })
      .where(eq(members.id, m.id))

    const found = await db.query.members.findFirst({
      where: eq(members.qrCode, 'qr-checkin'),
    })
  ...
  it('should search member by document number', async () => {
    ...
    await db
      .update(members)
      .set({ documentNumber: 'DOC-999' })
      .where(eq(members.id, m.id))

    const found = await db.query.members.findFirst({
      where: eq(members.documentNumber, 'DOC-999'),
    })
    expect(found).toBeDefined()
