...
describe('Notifications', () => {
  it('should create and verify', async () => {
    const n = await createNotification({ title: 'Bienvenido', message: 'Gracias por registrarte' })
  ...
  it('should filter by type', async () => {
    ...
    const payments = await db.query.notifications.findMany({ where: eq(notifications.type, 'PAYMENT') })
  ...
  it('should mark as read', async () => {
    ...
    const updated = await db.query.notifications.findFirst({ where: eq(notifications.id, n.id) })
  ...
  it('should count unread', async () => {
    ...
    const unread = await db.select({ cnt: count() }).from(notifications).where(eq(notifications.isRead, false))
    expect(unread[0]!.cnt).toBeGreaterThanOrEqual(1)
  })

  it('should create with reference', async () => {
    const n = await createNotification({ referenceId: 42, referenceType: 'subscription' })
